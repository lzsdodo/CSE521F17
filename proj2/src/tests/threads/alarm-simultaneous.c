...
/* Runs THREAD_CNT threads2 thread sleep ITERATIONS times each. */
static void
test_sleep (int thread_cnt, int iterations) 
{
  ...
  msg ("Creating %d threads2 to sleep %d times each.", thread_cnt, iterations);
  msg ("Each thread sleeps 10 ticks each time.");
  msg ("Within an iteration, all threads2 should wake up on the same tick.");
  ...
  /* Wait long enough for all the threads2 to finish. */
